...
其中 Qwen 和 Qwen-VL 的具体小型版本将在核对目标硬件能力后确定。YOLO11s 已使用 Raw6 部署边界完成 J6P 真实 COCO 校准、HBM 编译、5000 张板端精度评估和单核 Benchmark，INT8 mAP50-95 相对 FP32 下降 0.00813。掉点分析见 [YOLO11s J6P INT8 精度掉点分析与修复](docs/yolo11_j6p_int8_accuracy_recovery.md)。仓库自有 J6P C++ Runtime 已使用官方 OE `deps_aarch64` 完成交叉编译、真实板端运行、六路 Raw6 逐元素一致性、生产阈值检测结果一致性和端到端 Benchmark，因此状态为 `Verified`。其他模型目录当前只包含精简元数据。仓库不提交权重、数据集或大型产物。
